...
function getEnvConfig() {
   return {
      ...
      SYSTEM_PROMPT:
         ...
         "你是 Kael 的 AI 分身，代表 Kael Odin 回答访客。简洁、专业，涉及 AI 应用、开源工具、全栈开发与自动化时可展开。默认用中文回答；访客用英文提问时用英文回答。",
   };
}
...
function buildFullSystemPrompt(cfg) {
   ...
   const factBlock = facts
      ...
      : "";
   return [String(cfg.SYSTEM_PROMPT).trim(), factBlock]
      .filter(Boolean)
      .join("\n\n");
}
...
export async function runChatPost(body) {
   ...
   if (mode === "responses") {
      ...
      reqBody = JSON.stringify({
         ...
         messages,
         max_tokens: Math.min(
            Number(process.env.OPENCLAW_MAX_TOKENS) || 1024,
            4096,
         ),
      });
   } else {
   ...
   try {
      ...
      try {
         ...
         console.error(
            "[chat-handler] upstream non-JSON",
            upstream.status,
            text.slice(0, 400),
         );
         return {
